...
const LenPtHexStringCompress = 66;
...
function baseScarMulti(sk) {
    ...
    return curvePt.getEncoded(true);
}
...
function verifySig(random, sigS, rawMessage, pk) {
    ...
    console.log("----------------------in verifySig function -----------------");
...
//  pk          :hexstring
// return true,false
function verifySigCompress(sig, rawMessage, pk) {

    console.log("----------------------in verifySigCompress function -----------------");
    let random = removePrefix(sig).substring(0,66);
    let sigS = "0x"+(removePrefix(sig).substring(66));
...
function ptFromHexCompress(hexStr) {
    let hexStrTemp = removePrefix(hexStr);
    let bnX, bnY;
    if (hexStrTemp.length !== LenPtHexStringCompress) {
        throw ErrInvalidHexStringLen;
    }
    // bnX = BigInteger.fromBuffer(Buffer.from(hexStrTemp.substring(2, 66), 'hex'));
    // bnY = BigInteger.fromBuffer(Buffer.from(hexStrTemp.substring(66, LenPtHexStringCompress), 'hex'));

    //return Point.fromAffine(ecparams, bnX, bnY);
    return Point.decodeFrom(ecparams,Buffer.from(removePrefix(hexStr),'hex'))
}
...
module.exports = {
    ...
    verifySig:verifySig,
    verifySigCompress:verifySigCompress,
